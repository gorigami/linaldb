...
- **Value**: Enum representing all possible data-holding types: `Float(f32)`, `Float64(f64)`, `Int(i64)`, `String(String)`, `Bool(bool)`, `Vector(Vec<f32>)`, `Matrix(Vec<Vec<f32>>)`, `Complex(Complex64)`, `Null`. `Float64` (DSL keywords `DOUBLE`/`FLOAT64`) was added to fix a previously flagged gap — `Float` used to be the *only* floating-point representation anywhere in the engine, and large-magnitude real-world scalar values (GPS/Unix timestamps, etc.) silently lost precision. `Tensor`/`Vector`/`Matrix` remain `f32`-only by design — `Float64`/`Complex` are both scalar-only. Mixed-precision arithmetic (`Float op Float64`, `Int op Float64`) always promotes to `Float64`; any pairing touching `Complex` promotes to `Complex` (via `Value::as_complex()`, checked *before* the `Float64` promotion — `Complex` is strictly wider).
  - **`Complex(Complex64)`** (Phase 3 of `SCIENTIFIC_ENGINE_EXPANSION_PLAN.md`): wraps `num_complex::Complex64` directly (promoted from a transitive `nalgebra` dependency to a direct one). Scalar-only, per that plan's locked design decision — a genuine `Tensor<Complex>` is a separate, larger initiative. `Value::equals()` (distinct from `compare()`) gives `Complex` real `=`/`!=` semantics despite having no total order — see its doc comment for why `compare()`'s `Option<Ordering>` can't express "equal but unordered" on its own, and why every `=`/`!=`/`IN` call site now goes through `equals()` instead (unchanged behavior for every other type, which still ultimately derives from `compare()`).
- **ValueType**: the corresponding type descriptor used in schemas — `Float`, `Float64`, `Int`, `String`, `Bool`, `Vector(usize)`, `Matrix(usize, usize)`, `Complex`, `Null` (dimension-only, no data)
...
- **Adding `Complex` was the reference case for this codebase's "new `Value`/`ValueType` variant" discipline** (see the "Key design patterns" bullet on this in `CLAUDE.md`): `cargo build`'s exhaustive-match errors after adding the variant caught every direct match site, but a dedicated manual grep for `_ =>`/wildcard arms across `Value`/`ValueType` matches (which the compiler cannot flag) turned up 8 real, independently-verified bugs the compiler-driven pass missed entirely — `Value::compare()` silently reporting no order for `=`/`!=` instead of testing equality (fixed by adding a separate `Value::equals()` primitive), `ORDER BY` on a `Complex` column silently sorting as if every pair were tied, `MIN`/`MAX` aggregates (plain and windowed) silently freezing on the first row's value forever, `SUM`/`AVG` aggregates (plain and windowed) silently staying at `Int(0)`, and `AVG`'s schema type-inference silently reporting `Float` and causing a downstream type-mismatch error at execution time. Each was fixed to either work correctly (SUM/AVG, which are well-defined for `Complex`) or fail loudly with a clear error (MIN/MAX/ORDER BY, which are not) rather than return a plausible-looking wrong answer. Treat this as the template for auditing any future `Value`/`ValueType` variant addition — the compiler enumerates real match sites, but wildcard arms need a human.
...
- **`IndexDefinition`** (`column`, `index_type`): what `SAVE DATASET`/`LOAD DATASET` actually persist for indices — never the index contents themselves (`Dataset.indices` is `#[serde(skip)]`, since a `Box<dyn Index>`'s row-id contents are only meaningful paired with the exact rows it was built from). `LOAD DATASET` calls `create_index`/`create_vector_index` per persisted definition after the rows are back in place, which backfills from that data — reproducing an equivalent index without ever serializing `Box<dyn Index>`. Before this, every index was silently lost on reload with no warning.
  - **Vector index clustering persistence** (`SCIENTIFIC_ENGINE_EXPANSION_PLAN.md` Phase 2): `VectorIndex::snapshot()`/`restore_from_snapshot()` export/import the k-means clustering itself (`clusters`, `clustered_count`) as a `VectorIndexSnapshot`, via a new `Index::export_snapshot(&self) -> Option<serde_json::Value>` trait method (default `None` — only `VectorIndex` overrides it; `HashIndex` has no derived state worth persisting). `SAVE DATASET` writes each vector-indexed column's snapshot, paired with `VectorIndex::content_hash()` of that column's data, to `vector_index_clusters.json`. `LOAD DATASET` still backfills via `add()` from the freshly loaded rows (unchanged — this is what makes a snapshot's `members` indices-into-`vectors` still valid, since row order is preserved), but when a snapshot's content hash matches, calls `create_vector_index_from_snapshot` (`restore_from_snapshot`, skipping `build()`'s k-means pass entirely) instead of the from-scratch `create_vector_index`. A hash mismatch (e.g. `data.parquet` edited independently of the snapshot) falls back to the original full-rebuild path rather than trusting a stale clustering.
- **HnswIndex** (`PERFORMANCE_OPTIMIZATION_PLAN.md` Phase 1): a second `IndexType::Vector`-shaped index, `IndexType::Hnsw`, opted into via `CREATE VECTOR INDEX ... USING HNSW`. Backed by the `instant-distance` crate (pure Rust, no C/system dependency — chosen for that reason, matching this repo's build philosophy elsewhere). `build()` batch-constructs an HNSW graph (`instant_distance::HnswMap<CosinePoint, usize>`, values are row ids directly) once a column has ~16+ vectors (`MIN_VECTORS_TO_INDEX`); below that, or for vectors added after the last `build()`, both search paths brute-force scan -- the same "unindexed tail" guarantee `VectorIndex` gives, so correctness never depends on the graph being current. Only `search` (top-k) uses the graph: `search_threshold` (the exact `WHERE COSINE_SIM(...) > t` predicate) always brute-force scans every vector directly, never the graph, since an HNSW traversal -- unlike IVF's per-cluster angular-radius bound -- has no cheap provable bound on what it might have skipped, so it can't honor `search_threshold`'s "only skip what's provably excludable" contract. Consequently the planner's `CosineFilterExec`/`SimilarityJoinExec` acceleration paths stay `IndexType::Vector`-only; only `VectorSearchExec` (top-k `SEARCH`) accepts either index type. Persists via the same snapshot/content-hash mechanism as `VectorIndex` (`HnswIndexSnapshot`/`PersistedHnswIndex`, written to a sibling `hnsw_index_graphs.json`) -- but since `instant-distance`'s `HnswMap` serializes as a fully self-contained graph (its stored values *are* row ids, unlike `VectorIndex`'s `members` which are indices into a separately-maintained `vectors` buffer), restoring it doesn't depend on re-`add()`ing rows in a particular order, only on there being at least as many.

#### `storage.rs`

- **StorageEngine**: Trait for persistence abstraction
- **ParquetStorage**: Parquet-based dataset persistence. A dataset's on-disk package (`datasets/<name>/`) now also includes `indexes.json` — the serialized `Vec<IndexDefinition>` described above — and, when a vector index actually clustered, `vector_index_clusters.json` (its persisted `VectorIndexSnapshot` plus content hash, see below).
- Tensors persist via `ParquetStorage::save_tensor`, the same columnar format datasets use (this section previously described a separate `JsonStorage` for tensors; that type no longer exists in `src/core/storage.rs` — corrected here after re-verifying against current source while auditing `PERFORMANCE_OPTIMIZATION_PLAN.md`'s proposals).
...
- **`POST /execute`'s `?format=` options** (`PERFORMANCE_OPTIMIZATION_PLAN.md` Phase 3): `toon` (default, human-oriented text, not meant for programmatic parsing), `json` (opt-in, structured), and `arrow` (opt-in, binary Arrow IPC stream via `dataset_to_arrow_ipc_bytes`/`core::storage::dataset_to_record_batch`, for a successful `DslOutput::Table` result only -- anything else under `?format=arrow` falls back to a JSON body). Added after a real benchmark (`benches/server_transport.rs`) found Arrow IPC encoding ~80-180x faster and ~2.4x smaller on the wire than JSON for a representative 10k-row/`Vector(128)`-column result -- and, as a genuinely unexpected finding from the same benchmark, that the *default* `toon` encoding is itself substantially slower and larger than even the legacy `json` path for the same payload (measured ~426ms/30MB vs `json`'s ~36ms/13MB vs `arrow`'s ~233µs/5.5MB at 10k rows). That gap is not addressed here -- `toon`'s design goal is believed to be LLM-facing token efficiency, not wire/CPU efficiency, so whether it's actually a problem is a product judgment call for the maintainer, not something this change decided unilaterally. See `clients/CONTRACT.md` §1 for the wire-level contract `arrow` clients must follow.
...
- **Dataset Delivery** (`dataset_server.rs`): a separate `DatasetServer` router, mounted at `/delivery` in `server/mod.rs`, that serves a saved dataset package's files read-only over HTTP — `GET /delivery/datasets/:name/manifest.json`, `.../schema.json`, `.../stats.json`, `.../data.parquet` — the same four files `storage.rs`'s `save_dataset_package` writes to disk (see Storage Layer). This is what `DELIVER <dataset>` (DSL_REFERENCE.md §9) checks the readiness of. Resolves the per-database subdirectory (`{data_dir}/{database}/datasets/:name/...`) via the same `X-Linal-Database` header `/execute` honors, default `"default"` — **fixed in v0.1.73**: every handler here previously read `{data_dir}/datasets/:name/...`, missing that segment, so `/delivery` 404'd for every dataset saved through the real `SAVE DATASET` path (found building the Python client, `clients/python/linaldb_server/dataset.py`).
- **Database Management API**: `GET /databases`, `POST /databases/:name`, `DELETE /databases/:name`.
- **Per-database locking** (`server/engine.rs`, `SharedEngine`): each database is its own single-database `TensorDb` (`TensorDb::from_instance`) behind its own `RwLock`, so requests against different databases never wait on each other.
  - Catalog statements (`CREATE`/`DROP`/`USE DATABASE`, `SHOW DATABASES`) are answered by `SharedEngine` itself. Everything else runs on the target database's engine.
  - It takes a read lock when `dsl::can_execute_shared` allows it (read-only statements, `SHOW` except on a lazy tensor, `DESCRIBE PIPELINE`) and a write lock otherwise.
  - The pipeline registry (`PipelineRegistry`) is shared by all databases, so pipelines stay session-wide.
  - The catalog lock is never held while a database lock is taken.
  - The request's target is modeled as a `Session`:
    - `Pinned`: a header, or a job.
    - `Server`: a headerless request.
    - `Following`: a headerless batch.
  - This replaced one global `Arc<RwLock<TensorDb>>`, where every non-read-only statement (including `SELECT`) locked out all databases (see CHANGELOG).
- **Multi-tenant Isolation**: Isolated database contexts via `X-Linal-Database` header. A request that carries the header is pinned to that database for its whole duration and never touches the server's active database, so concurrent requests targeting different databases via the header cannot affect each other's context. **A headerless request's own active-database changes now persist** (fixed in v0.1.74): a `USE <db>` statement sent to `/execute` with no `X-Linal-Database` header used to report success but have its effect silently reverted by this same restore logic — which ran unconditionally regardless of whether the request had asked to switch databases at all — so the session-level `USE` workflow was a no-op over HTTP (every subsequent headerless request, no matter how many, kept seeing the old active database), even though the identical command works correctly via the embedded CLI/REPL. The restore now only fires for a request that itself supplied the header.
- **`USE` + header is a rejected combination, not a silent revert**: the v0.1.74 fix above only closed the *headerless* case. A `USE <db>` statement sent to `/execute` or `/jobs` *with* `X-Linal-Database` set had the identical bug — the response claimed `"Switched to database 'x'"`, but the switch was reverted by the same restore-after-request logic before the response went out, since that logic guards *every* header-bearing request unconditionally, regardless of whether the statement it ran was itself a `USE`. Confirmed against a live server: no client or test ever relied on this combination (both `clients/python` and `clients/r` always send the header on every request and never rely on `USE` persisting across calls), so rather than give it real persistence semantics — which would mean tracking active-database state per header value instead of one process-wide field, reopening the cross-tenant leak class the header mechanism exists to prevent — `/execute` and `/jobs` now reject a header-bearing request whose statement is `USE` outright, before touching `active_db` at all. See `POST /execute/batch` below for where `USE` legitimately belongs.
- **`POST /execute/batch`**: executes a whole multi-statement script (same line-joining rules as `linal run`, extracted into the shared `dsl::script::split_script`) as one request, running every statement in order and stopping at the first error. Consecutive statements on the same database run under one write-lock hold on that database.
  - A `USE` inside the batch persists for the rest of *that* batch.
  - A header-bearing batch (`Session::Pinned`) never lets that `USE` leave the batch, so there's no cross-tenant leak.
  - A headerless batch (`Session::Following`) also moves the server's active database, like a headerless `/execute` `USE`. This is the endpoint a script that needs `USE` to control several subsequent statements should use instead of separate `/execute` calls.
- **`/schedule`'s `target_db` switch is permanent, by design, not a bug**: unlike `/execute`/`/execute/batch`, a scheduled task's database switch (`scheduler.rs`) is never restored — a recurring task is operator-configured at creation time, not per-visitor request traffic, so there's no "previous" per-request context to protect. This means a scheduled task with `target_db` set changes the session-wide active database for every subsequent headerless request until something else changes it again; mixing scheduled tasks with headerless `/execute` traffic on the same server should account for this.
...
- **OpenAPI/Swagger documentation**: Interactive API explorer at `/swagger-ui`. Note: only `/execute`, `/execute/batch`, and `/health` are currently included in the generated schema; all other routes are functional but undocumented in the spec.
