...
- **`POST /execute/batch`**: executes a whole multi-statement script (same line-joining rules as `linal run`, extracted into the shared `dsl::script::split_script`) as one request, under one write-lock hold. A header-bearing batch snapshots the previously active database once, runs every statement in order (stopping at the first error), then restores it once at the end — so a `USE`/`CREATE DATABASE` statement inside the batch persists naturally for the rest of *that* batch (nothing restores mid-batch) without ever risking a cross-tenant leak, since its effect can't outlive the batch's own lock hold. This is the endpoint a script that needs `USE` to control several subsequent statements should use instead of separate `/execute` calls.
